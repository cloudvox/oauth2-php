...
abstract class Client
{
    ...
    /**
     * Initialize a OAuth2.0 Application.
     *
     ...
     *
     * By default we generate session object based on access_token response, or if it is provided from server with
     * $_REQUEST. For sure, if it is provided by server it should follow our session object format.
     *
     * Session object provided by server can ensure the correct expires and base_domain setup as predefined in server,
     * also you may get more useful information for custom functionality, too. BTW, this may require for additional
     ...
     *
     * @param null $access_token(optional) A valid access token in associative array as below:
     ...
     * @return mixed|null A valid session object in associative array for setup cookie, and null if not able to
     * generate it with custom method.
     */
    protected function getSessionObject($access_token = null)
    {
        ...
        if (!empty($access_token) && isset($access_token['access_token'])) {
            ...
            $session['expires'] = isset($access_token['expires_in']) ? time() + $access_token['expires_in']
                : time() + $this->getVariable('expires_in', self::DEFAULT_EXPIRES_IN);
     ...
     *
     * If no valid OAuth2.0 access token found in session object, this function will automatically switch as normal
     * remote API call without "oauth_token" parameter.
     *
     * Assume server reply in JSON object and always decode during return. If you hope to issue a raw query, please use
     * makeRequest().
     *
     ...
     * Set the Session.
     *
     * @param array $session (optional) The session object to be set. null if hope to flush existing session object.
     * @param bool $write_cookie (optional) true if a cookie should be written. This value is ignored if cookie support
     * has been disabled.
     * @return Client The current OAuth2.0 client-side instance.
     */
     ...
     *
     * @return string
     * The valid session object with OAuth2.0 information, and null if not able to discover any cases.
     */
        ...
        if (!$this->getVariable('_session_loaded')) {
            ...
            if (!$session && $this->getVariable('cookie_support')) {
                ...
                if (isset($_COOKIE[$cookie_name])) {
                    ...
                    parse_str(
                        trim(
                            get_magic_quotes_gpc() ? stripslashes($_COOKIE[$cookie_name]) : $_COOKIE[$cookie_name],
                            '"'
                        ),
                        $session
                    );
     ...
     *
     * This function will only be activated if both access token URI, client identifier and client secret are setup
     * correctly.
     *
     * @param string $code Authorization code issued by authorization server's authorization endpoint.
     * @return mixed|null A valid OAuth2.0 JSON decoded access token in associative array, and null if not enough
     * parameters or JSON decode failed.
     */
    private function getAccessTokenFromAuthorizationCode($code)
    {
        if ($this->getVariable('access_token_uri') && $this->getVariable('client_id')
     ...
     * @param string $method (optional) The HTTP method (default 'GET').
     * @param array $params (optional) The GET/POST parameters.
     * @param resource $curl_handle (optional) An initialized curl handle
     * @return string The JSON decoded response object.
     * @throws \OAuth2\Exception\Exception
     */
    protected function makeRequest($path, $method = 'GET', $params = array(), $curl_handle = null)
    {
        if (!$curl_handle) {
        ...
        if (curl_errno($curl_handle) == 60) { // CURLE_SSL_CACERT
            error_log('Invalid or no certificate authority found, using bundled information');
            curl_setopt($curl_handle, CURLOPT_CAINFO, dirname(__FILE__) . '/fb_ca_chain_bundle.crt');
            $result = curl_exec($curl_handle);
        }

        if ($result === false) {
            $e = new Exception(array('code' => curl_errno($curl_handle), 'message' => curl_error($curl_handle)));
     ...
     */
    protected function setCookieFromSession($session = array())
    {
        ...
        if ($session) {
            ...
            $expires = isset($session['expires']) ? $session['expires']
                : time() + $this->getVariable('expires_in', self::DEFAULT_EXPIRES_IN);
        }
    ...
    /**
     * Since $_SERVER['REQUEST_URI'] is only available on Apache, we generate an equivalent using other environment
     * variables.
     *
     * @return string
     */
     ...
     * @return null|string The URL for the given parameters.
     */
